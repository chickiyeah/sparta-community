...
const uploadImage = async (uri) => {
  ...
  return ready ? <Loading/> :   (
    ...
    />
        ...
        <Button title="수정하기" disabled={!isChecked} onPress={() => {Alert.alert("수정 확인","정말 수정하시겠습니까?", [
        {
            text:'취소',
            onPress: () => Alert.alert('수정이 취소되었습니다.', '사용자가 수정을 취소했습니다.'),},
        {
            text:'확인',
            onPress: () => {
                if(Name != ""){
                        ...
                        if(Desc != ""){
                            if(image != null){
                                ...
                                firebase_db.ref(`/tip/${index}`).set(newtip,function(error){
                                    console.log(error)
                                    Alert.alert("수정 완료!", "성공적으로 꿀팁을 수정했습니다!",[
                                      {
                                        text:'확인',
                                        onPress: () => gohome()}
                                    ])
